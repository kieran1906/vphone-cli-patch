...
function frameFromDesc(desc) {
  const m2 = desc.match(/frame\s*=\s*\(([\d.+-]+)\s+([\d.+-]+);\s*([\d.+-]+)\s+([\d.+-]+)\)/);
  if (m2) {
    const [, x, y, w, h] = m2.map(Number);
    if (w > 0 && h > 0)
...
function isOnScreen(rect) {
  return rect.w > 0 && rect.h > 0 && rect.x >= 0 && rect.x <= SCREEN_W && rect.y >= 0 && rect.y <= SCREEN_H;
}
rpc.exports = {
    ...
    return new Promise((resolve) => {
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, () => {
        try {
          let bundleID = "";
          try {
            const ctrl = frida_objc_bridge_default.classes.SBApplicationController["sharedInstance"]();
          ...
          if (!bundleID) {
            try {
              const pm = frida_objc_bridge_default.classes.FBProcessManager["sharedInstance"]();
          ...
          resolve({ bundleID: bundleID || void 0 });
        } catch (err) {
  ...
  diagnose() {
    return new Promise((resolve) => {
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, () => {
        const info = {};
        try {
          info.sbIconViewCount = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.SBIconView).length;
        } catch (e) {
          info.sbIconViewErr = String(e);
        }
        try {
          info.uiLabelCount = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.UILabel).length;
        } catch (e) {
          info.uiLabelErr = String(e);
        }
        try {
          const icons = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.SBIconView);
          if (icons.length > 0) {
            info.icon0label = icons[0].accessibilityLabel().toString();
            info.icon0frame = icons[0].accessibilityFrame().toString();
          }
        } catch (e) {
          info.icon0err = String(e);
        }
        resolve(info);
      });
    });
  ...
  dumpElements() {
    return new Promise((resolve) => {
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, () => {
        try {
          let add2 = function(ptr2, label, hint, rect, source) {
            if (seenPtrs.has(ptr2) || !label || !isOnScreen(rect))
              return;
            seenPtrs.add(ptr2);
            out.push({ label, hint, ...rect, source });
          }, bestFrame2 = function(obj) {
            const af = frameFromAxFrame(obj);
            if (af)
              return af;
            return frameFromDesc(descOf(obj));
          }, isVisible2 = function(obj) {
            try {
              if (obj.isHidden())
                return false;
            } catch (_) {
            ...
            try {
              if (Number(obj.alpha()) < 0.01)
                return false;
            } catch (_) {
            }
            return true;
          }, walkForAxLabels2 = function(obj, depth) {
            if (depth > 15)
              return;
            try {
              const ptr2 = obj.handle.toString();
              if (seenPtrs.has(ptr2))
                return;
              if (!isVisible2(obj))
                return;
              const axLabel = strOf(obj.accessibilityLabel());
              if (axLabel) {
                const rect = bestFrame2(obj);
                if (rect)
                  add2(ptr2, axLabel, strOf(obj.accessibilityHint()), rect, "ax");
              }
              const subs = obj.subviews();
              const n = toNum(subs.count());
              for (let i = 0; i < n; i++) {
                walkForAxLabels2(subs.objectAtIndex_(i), depth + 1);
              }
            } catch (_) {
            }
          };
          var add = add2, bestFrame = bestFrame2, isVisible = isVisible2, walkForAxLabels = walkForAxLabels2;
          const out = [];
          const seenPtrs = /* @__PURE__ */ new Set();
          try {
            const labels = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.UILabel);
            for (const lbl of labels) {
              try {
                ...
                if (!text)
                  continue;
                const rect = bestFrame2(lbl);
                if (!rect)
                  continue;
                const axLabel = strOf(lbl.accessibilityLabel());
                add2(lbl.handle.toString(), axLabel || text, "", rect, "label");
              } catch (_) {
              }
            }
          } catch (_) {
          ...
          try {
            const buttons = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.UIButton);
            for (const btn of buttons) {
              try {
                if (!isVisible2(btn))
                  continue;
                const rect = bestFrame2(btn);
                if (!rect)
                  continue;
                let label = strOf(btn.accessibilityLabel());
                if (!label) {
                  try {
                    label = strOf(btn.titleLabel().text());
                  } catch (_) {
                  }
                }
                if (!label)
                ...
                add2(btn.handle.toString(), label, hint, rect, "button");
              } catch (_) {
              }
            }
          } catch (_) {
          ...
          try {
            const fields = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.UITextField);
            for (const f2 of fields) {
              try {
                if (!isVisible2(f2))
                  continue;
                const rect = bestFrame2(f2);
                if (!rect)
                  continue;
                const label = strOf(f2.accessibilityLabel()) || strOf(f2.placeholder());
                if (!label)
                  continue;
                add2(f2.handle.toString(), label, "", rect, "textfield");
              } catch (_) {
              }
            }
          } catch (_) {
          ...
          try {
            const iconViews = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.SBIconView);
            for (const iv of iconViews) {
              try {
                ...
                if (!isOnScreen(rect))
                  continue;
                add2(iv.handle.toString(), label, "", rect, "icon");
              } catch (_) {
              }
            }
          } catch (_) {
          ...
          try {
            const wins = frida_objc_bridge_default.chooseSync(frida_objc_bridge_default.classes.UIWindow);
            for (const w of wins) {
              try {
                if (w.isHidden())
                  continue;
