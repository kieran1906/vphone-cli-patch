...
// agent/screenshot.ts
function findFn(name, retType2, argTypes2) {
  for (const m2 of Process.enumerateModules()) {
    try {
      for (const e of m2.enumerateExports()) {
        if (e.name === name)
          return new NativeFunction(e.address, retType2, argTypes2);
      }
    } catch (_) {
    }
  }
...
var createScreenImage = findFn("_UICreateScreenUIImage", "pointer", []);
var pngRepFn = findFn("UIImagePNGRepresentation", "pointer", ["pointer"]);
var mat = findFn("mach_absolute_time", "uint64", []);
var createFinger = findFn("IOHIDEventCreateDigitizerFingerEvent", "pointer", ["pointer", "uint64", "uint32", "uint32", "uint32", "double", "double", "double", "double", "double", "int32", "uint32"]);
var dispatchHID = findFn("BKSHIDEventSendToFocusedProcess", "void", ["pointer"]);
var MASK_BEGIN = 7;
var MASK_MOVE = 4;
var MASK_UP = 3;
var SCREEN_W = 430;
var SCREEN_H = 932;
function sendTouch(nx, ny, mask, pressure) {
  if (!mat || !createFinger || !dispatchHID)
    return;
  const ts = mat();
  const ev = createFinger(ptr("0"), ts, 1, 1, mask, nx, ny, 0, pressure, 0, 1, 0);
  dispatchHID(ev);
}
function scheduleTouch(nx, ny, mask, pressure) {
...
rpc.exports = {
    ...
    return new Promise((resolve) => {
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, () => {
        try {
          ...
          const imgPtr = createScreenImage();
          if (imgPtr.isNull()) {
            resolve({ ok: false, path: outPath, error: "null image" });
            return;
          }
          const dataPtr = pngRepFn(imgPtr);
          if (dataPtr.isNull()) {
            resolve({ ok: false, path: outPath, error: "null data" });
            return;
          }
          const data = new frida_objc_bridge_default.Object(dataPtr);
          data.writeToFile_atomically_(outPath, 1);
          resolve({ ok: true, path: outPath, bytes: data.length() });
        } catch (err) {
  ...
  async tap(x, y) {
    const nx = x / SCREEN_W;
    const ny = y / SCREEN_H;
    await scheduleTouch(nx, ny, MASK_BEGIN, 1);
    await new Promise((r) => setTimeout(r, 100));
    await scheduleTouch(nx, ny, MASK_MOVE, 1);
    await new Promise((r) => setTimeout(r, 50));
    await scheduleTouch(nx, ny, MASK_UP, 0);
    return true;
  },
  async swipe(x1, y1, x2, y2, steps) {
    steps = steps || 20;
