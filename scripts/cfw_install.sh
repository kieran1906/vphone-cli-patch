...
# ── Sudo helper (non-interactive via VPHONE_SUDO_PASSWORD) ───────
VPHONE_SUDO_PASSWORD="${VPHONE_SUDO_PASSWORD:-}"
_sudo() {
    if [[ -n "$VPHONE_SUDO_PASSWORD" ]]; then
        echo "$VPHONE_SUDO_PASSWORD" | sudo -S "$@"
    else
        sudo "$@"
    fi
}
...
safe_detach() {
    ...
    if mount | grep -Fq " on $mnt "; then
        _sudo hdiutil detach -force "$mnt" 2>/dev/null || true
    fi
}
...
if [[ "${CRYPTEX_OS_COUNT:-0}" -gt 0 && "${CRYPTEX_APP_COUNT:-0}" -gt 0 ]]; then
    ...
    _sudo hdiutil attach -mountpoint "$MNT_SYSOS" "$SYSOS_DMG" -nobrowse -owners off
    echo "  Mounting AppOS..."
    _sudo hdiutil attach -mountpoint "$MNT_APPOS" "$APPOS_DMG" -nobrowse -owners off
