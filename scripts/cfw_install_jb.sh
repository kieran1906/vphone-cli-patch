...
# Deploy persistent sshd starter — runs on every boot via launchd KeepAlive
SSHD_START_SCRIPT="$SCRIPT_DIR/vphone_sshd_start.sh"
if [[ -f "$SSHD_START_SCRIPT" ]]; then
    scp_to "$SSHD_START_SCRIPT" "/mnt1/cores/vphone_sshd_start.sh"
    ssh_cmd "/bin/chmod 0755 /mnt1/cores/vphone_sshd_start.sh"
...
with open(sys.argv[1], 'rb') as f:
    target = plistlib.load(f)
sshd_daemon = {
    'Label': 'com.vphone.sshd',
    'ProgramArguments': ['/cores/vphone_sshd_start.sh'],
    'RunAtLoad': True,
    'KeepAlive': True,
}
target.setdefault('LaunchDaemons', {})['com.vphone.sshd'] = sshd_daemon
with open(sys.argv[1], 'wb') as f:
    plistlib.dump(target, f, sort_keys=False)
" "$TEMP_DIR/launchd.plist"
    scp_to "$TEMP_DIR/launchd.plist" "/mnt1/System/Library/xpc/launchd.plist"
    ssh_cmd "/bin/chmod 0644 /mnt1/System/Library/xpc/launchd.plist"
    echo "  [+] com.vphone.sshd injected into launchd.plist (persistent across reboots)"
fi

# NOTE: Setup wizard bypass (JB-6) removed — mounting the data volume (disk1s2)
# requires SEP/gigalocker-init which hangs in the VM (no effaceable storage).
# Complete the iOS setup wizard manually on first boot.
