...
if [[ -x "$TRUSTCACHE_BIN" ]]; then
    ...
    make -C "$BUILD_DIR/trustcache" \
        OPENSSL=1 \
        CFLAGS="-isysroot $SDK_PATH -I$OPENSSL_PREFIX/include -DOPENSSL -w" \
...
if [[ -x "$INSERT_DYLIB_BIN" ]]; then
    ...
    mkdir -p "$TOOLS_PREFIX/bin"
    clang -isysroot "$(xcrun --show-sdk-path)" -o "$INSERT_DYLIB_BIN" "$INSERT_DYLIB_DIR/insert_dylib/main.c" -framework Security -O2
